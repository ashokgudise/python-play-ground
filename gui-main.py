...
# Includes: Parameter, Selection (if-else), and Iteration (while loop)
def process_move(choice_made):
    ...
    if is_correct:
        ...
        # 2. Iteration: WHILE LOOP (AP CSP Requirement)
        # This loop runs to rebuild the history string based on progress.
        history_log = "" # Clear and rebuild
        counter = 1
        while counter <= scenario_number:
            history_log = history_log + "Room " + str(counter) + " OK! "
            counter = counter + 1
...
def update_display():
    ...
    if scenario_number > 5:
        win_text = "VICTORY " + player_name.upper() + "!\nFinal Score: " + str(score)
        canvas.create_text(300, 200, text=win_text, fill="gold", font=("Arial", 16, "bold"))
        win_button = tk.Button(root, text="Play Again", command=show_main_menu)
        ...
        # Show the log generated by the while loop
        canvas.create_text(300, 430, text=history_log, fill="#AFAFAF", font=("Arial", 10), width=500)

def game_over_message(reason):
    canvas.delete("all")
    full_msg = "Sorry " + player_name + ",\n" + reason + "\nFinal Score: " + str(score)
    canvas.create_text(300, 200, text=full_msg, fill="red", font=("Arial", 14), width=400, justify="center")
